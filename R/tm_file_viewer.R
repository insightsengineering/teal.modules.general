...
tm_file_viewer <- function(label = "File Viewer Module",
  ...
  stop_if_not(
    ...
    vapply(input_path, function(x)
      file.exists(x) || valid_url(x), logical(1))
  )

  if (!is.list(input_path)) {
    input_path <- list(input_path)
  } else if (all(vapply(input_path, function(x)
    utils::file_test("-d", x), logical(1)))) {
  ...
  function(input,
    ...
    handle_connection_type <- function(file_path) {
      ...
      if (class(file_class)[1] == "url") {
        ...
        if (isFALSE(output_text[1]) || file_extension == "svg") {
          file.copy(normalizePath(file_path, winslash = "/"),
                    temp_dir)
          file_path <-
            file.path(basename(temp_dir), basename(file_path))
        }
    ...
    display_file <- function(file_path) {
      ...
      if (file_extension %in% c("png", "apng", "jpg", "jpeg", "svg", "gif", "webp", "bmp")) {
        ...
        tags$embed(style = "height:600px; width:100%",
