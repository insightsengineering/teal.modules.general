...
Here's an example to showcase how you can edit an output of class `patchwork`.
Since `patchwork` objects support ggplot2-style addition, you can use `patchwork::plot_annotation()` and
standard ggplot2 theme modifications.

```{r decorate_patchwork, message=FALSE}
library(teal.modules.general)
...
code <- paste0(c(
  "interactive <- function() TRUE",
  knitr::knit_code$get("setup"),
  knitr::knit_code$get("decorate_patchwork")
), collapse = "\n")
