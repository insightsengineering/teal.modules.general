...
| `tm_g_scatterplotmatrix` | plot (ggmatrix)                                 |
...
Here's an example to showcase how you can edit an output of class `ggmatrix`
(produced by `GGally::ggpairs`).

```{r decorate_ggmatrix, message=FALSE}
library(teal.modules.general)
...
ggmatrix_title_decorator <- function(default_title = "I am a good decorator") {
  teal_transform_module(
    label = "Title",
...
app <- init(
  ...
  modules = modules(
    tm_g_scatterplotmatrix(
      ...
      decorators = list(
        plot = ggmatrix_title_decorator("I am a Scatterplot matrix")
      )
    )
  )
)
...
code <- paste0(c(
  "interactive <- function() TRUE",
  knitr::knit_code$get("setup"),
  knitr::knit_code$get("decorate_ggmatrix")
), collapse = "\n")
