...
testthat::test_that("e2e - tm_variable_browser: content is displayed correctly.", {
  ...
  # Test tab name
  testthat::expect_equal(app_driver$get_text(".teal-modules-tree .active"), "Variable browser test")

  # Plot is visible
  testthat::expect_true(
    app_driver$is_visible(
      app_driver$namespaces(TRUE)$module("variable_plot-plot_out_main .shiny-plot-output")
    )
  )
  ...
  testthat::expect_setequal(
    trimws(app_driver$get_text(
      app_driver$namespaces(TRUE)$module("ui_variable_browser .nav li")
    )),
  ...
  testthat::expect_contains(
    trimws(
      app_driver$get_text(
        app_driver$namespaces(TRUE)$module("variable_summary_table table th")
      )
    ),
    "Statistic"
  )
...
testthat::test_that("e2e - tm_variable_browser: Selecting 'treat variable as factor' changes the table headers.", {
  ...
  current_var <- trimws(app_driver$get_text(
    app_driver$namespaces(TRUE)$module("ui_variable_browser li .active")
  ))
...
testthat::test_that("e2e - tm_variable_browser: selection of categorical variable has a table with 'level' header.", {
  ...
  categorical_selector <- app_driver$namespaces(TRUE)$module(
    sprintf(
...
testthat::test_that("e2e - tm_variable_browser: changing 'display density' encoding doesn't show errors.", {
  ...
  app_driver$click(selector = app_driver$namespaces(TRUE)$module("display_density"))
  app_driver$expect_no_shiny_error()
  app_driver$click(selector = app_driver$namespaces(TRUE)$module("display_density"))
  app_driver$expect_no_shiny_error()
...
testthat::test_that("e2e - tm_variable_browser: changing 'outlier definition' encoding doesn't show errors.", {
  ...
  testthat::expect_null(
    app_driver$get_text(
      app_driver$namespaces(TRUE)$module("outlier_definition_slider")
    )
  ) ## Does not exist initially

  app_driver$click(selector = app_driver$namespaces(TRUE)$module("remove_outliers"))
  app_driver$expect_no_shiny_error()
