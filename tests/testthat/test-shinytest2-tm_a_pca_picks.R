    ...
    teal::init(
      ...
      modules = tm_a_pca(
        dat = teal.picks::picks(
          teal.picks::datasets("USArrests", "USArrests"),
          teal.picks::variables(c("Murder", "Assault", "UrbanPop", "Rape"), c("Murder", "Assault"))
        ),
        size = c(3, 1, 5),
        alpha = c(.5, 0, 1),
        font_size = c(10, 8, 15),
...
testthat::test_that("e2e - tm_a_pca: Module is initialised with the specified defaults in function call.", {
  ...
  testthat::expect_setequal(
    app_driver$get_active_module_input("dat-variables-selected"),
    c("Murder", "Assault")
  )
...
testthat::test_that("e2e - tm_a_pca: Color by columns (data_extract) must be from non-selected variable set.", {
  ...
  app_driver$set_active_module_input("response", c("UrbanPop"))
  app_driver$expect_no_validation_error()

  app_driver$set_active_module_input("response", c("Murder"))
...
testthat::test_that("e2e - tm_a_pca: Changing output encodings of 'plot_type' hides and shows options.", {
  ...
  color_by_selector <- app_driver$namespaces(TRUE)$module("response")
